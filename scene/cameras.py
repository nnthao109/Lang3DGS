...
class Camera(nn.Module):
    ...
    def get_language_feature_sam(self, language_feature_dir, feature_level):
    ...
    def get_language_feature(self, language_feature_dir, feature_level):
        language_feature_name = os.path.join(language_feature_dir, self.image_name)
        seg_map = torch.from_numpy(np.load(language_feature_name + '_s.npy'))  # Now of shape [876, 1332]
        feature_map = torch.from_numpy(np.load(language_feature_name + '_f.npy'))
        
        y, x = torch.meshgrid(torch.arange(0, self.image_height), torch.arange(0, self.image_width))
        x = x.reshape(-1, 1)
        y = y.reshape(-1, 1)
        seg = seg_map[y, x].squeeze(-1).long()  # No need to index on the first dimension now
        mask = seg != -1
        
        # Since there's no feature_level distinction anymore, we just use `seg` directly
        point_feature = feature_map[seg].squeeze(0)
        mask = mask.reshape(1, self.image_height, self.image_width)
        
        point_feature = point_feature.reshape(self.image_height, self.image_width, -1).permute(2, 0, 1)
