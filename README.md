...
- **Step 2: Train the Autoencoder and get the lower-dims Feature.**
  ...
  To reduce the dimensionality of CLIP embeddings, train the autoencoder as follows:
...
## Purpose
The goal is to compress the CLIP feature embeddings into 32 dimensions.

## Modifications

### Modify Scene Initialization
In `scene/gaussian_model.py`, modify the initialization to include a 32-dimensional language feature:
...
if training_args.include_feature:
    if self._language_feature is None or self._language_feature.shape[0] != self._xyz.shape[0]:
        language_feature = torch.zeros((self._xyz.shape[0], 32), device="cuda")  # Change 32 to the desired number of dimensions
        self._language_feature = nn.Parameter(language_feature.requires_grad_(True))
```

### Update Number of Channels for Rasterization
In `LangSplat/submodules/langsplat-rasterization/cuda_rasterizer/config.h`, update the number of channels for language features:

```c
// LangSplat/submodules/langsplat-rasterization/cuda_rasterizer/config.h
#define NUM_CHANNELS 3 // For RGB
#define NUM_CHANNELS_language_feature 32 // For CLIP features
```

### Rebuild and Reinstall
Rebuild the rasterization module and reinstall it:

```bash
rm -rf /LangSplat/submodules/langsplat-rasterization/build
...
- **Step 4: Render the LangSplat.**
  ...
  python render_clip.py -s dataset_path -m output/${casename} --feature_level ${level} # using CLIP model 
  python view_viser.py -s dataset_path -m output/${casename} --feature_level ${level} # visualization
  ```
- **Step 5: Eval.**
