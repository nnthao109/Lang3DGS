...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if opt.include_feature:
            ...
            # print("Shape",language_feature.shape, gt_language_feature.shape )
            Ll1 = l1_loss(language_feature*language_feature_mask, gt_language_feature*language_feature_mask)  
            gt_image = viewpoint_cam.original_image.cuda()
            Ll1_ = l1_loss(image, gt_image)
            loss = (1.0 - opt.lambda_dssim) * Ll1_ + opt.lambda_dssim * (1.0 - ssim(image, gt_image))  + 2*Ll1       
            # loss = Ll1
        else:
            gt_image = viewpoint_cam.original_image.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[7_000, 30_000, 50_000])
