...
class OpenCLIPNetwork(nn.Module):
    ...
    def extract_text_feature(self, text_list):
        with torch.no_grad():
            tok_phrases = torch.cat([self.tokenizer(phrase) for phrase in text_list]).to("cuda")
            text_features = self.model.encode_text(tok_phrases)
...
def _embed_clip_sam_tiles(image, sam_encoder):
    ...
    print(f"sam_encoder {end_time - start_time:.6f} seconds")
    ...
    for mode in ['default', 's', 'm', 'l']:
        ...
        with torch.no_grad():
            ...
            end_time = time.time()  # End time tracking
            print(f"clip_encode_image {end_time - start_time:.6f} seconds")
