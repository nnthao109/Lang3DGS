...
def create(image_list, data_list, save_folder, model, mask_generator):
    ...
    for i, img in tqdm(enumerate(image_list), desc="Embedding images", total=len(image_list)):
        try:
            img_embed, seg_map = embed_clip_sam(img, model, mask_generator)
...
def embed_clip_sam(image, clip_model, mask_generator):
    image_np = image.permute(1, 2, 0).cpu().numpy().astype(np.uint8)
    image_rgb = cv2.cvtColor(image_np, cv2.COLOR_BGR2RGB)
    masks = mask_generator.generate(image_rgb)

    # Handle different structures of masks
    if isinstance(masks, dict):
        masks = masks.get('masks', [])
    elif masks and isinstance(masks[0], list):
        masks = [mask for sublist in masks for mask in sublist]

    if not masks:
        raise RuntimeError("No masks generated by SAM.")

    # Ensure masks is a list of dictionaries
    if not isinstance(masks[0], dict):
        raise RuntimeError(f"Unexpected mask format: {type(masks[0])}")

    # Sort masks by area (descending)
    masks = sorted(masks, key=lambda x: x['area'], reverse=True)

    # Limit the number of masks to process
    max_masks = 100
...
def main():
    ...
    create(images, data_list, save_folder, clip_model, mask_generator)
